...
================================================================
Author: Jindřich Brož — Licensed under the Apache License 2.0 (see LICENSE).
...
# Disable torch.compile / _dynamo in frozen builds — it can't JIT-compile anyway
# and its import chain fails under PyInstaller (TensorifyState etc.)
if getattr(sys, 'frozen', False) and _HAS_TORCH:
    try:
        import torch._dynamo
        torch._dynamo.config.suppress_errors = True
        torch._dynamo.disable()
    except Exception:
        # _dynamo itself is broken — monkey-patch torch.compile to a no-op
...
try:
    ...
    _nnunet_err = f"nnUNet import failed: {_e}\n{_tb.format_exc()}"
    print(_nnunet_err, file=sys.stderr)
    # Also write to log file so we can diagnose in .exe builds
    _log_path = os.path.join(os.path.dirname(os.path.abspath(sys.executable if getattr(sys, 'frozen', False) else __file__)), "MSLesionTool_log.txt")
    try:
        with open(_log_path, "a") as _lf:
            _lf.write(_nnunet_err + "\n")
    except Exception:
        pass
...
# User-local model cache (%APPDATA%/MSLesionTool on Windows, ~/.mslesion on others)
...
_logger_early = logging.getLogger("MSLesionTool")
_logger_early.info("_APP_DIR = %s", _APP_DIR)
_logger_early.info("_USER_MODEL_DIR = %s", _USER_MODEL_DIR)
_logger_early.info("_MODEL_SEARCH_DIRS = %s", _MODEL_SEARCH_DIRS)
...
def _resolve_model_dir(subdir):
    ...
    for base in _MODEL_SEARCH_DIRS:
        ...
        _logger_early.info("  resolve_model_dir: %s -> exists=%s", candidate, exists)
        if exists:
            return candidate
    return None


def _check_models_missing():
    """Return list of (arch_key, fold) pairs whose checkpoint is not on disk."""
    missing = []
    for arch_key, (subdir, _ptype, _display) in ARCHITECTURES.items():
        for fold in DEFAULT_BEST2[arch_key]:
            d = _resolve_model_dir(subdir)
...
class ModelDownloadThread(QThread):
    ...
    def __init__(self, missing_models, parent=None):
        super().__init__(parent)
        self._missing = missing_models

    def _download_file(self, rel_path, local_path):
        """Download a file from HuggingFace via plain HTTP."""
        import urllib.request
        url = f"https://huggingface.co/{HF_REPO_ID}/resolve/main/{rel_path}"
        os.makedirs(os.path.dirname(local_path), exist_ok=True)
        urllib.request.urlretrieve(url, local_path)

    def run(self):
        msseg_dir = os.path.join(_USER_MODEL_DIR, "msseg")
        total = len(self._missing)

        try:
            # Download config files for each architecture
            needed_archs = set(arch for arch, _fold in self._missing)
            config_files = ["dataset.json", "plans.json", "dataset_fingerprint.json"]

            for arch_key in needed_archs:
                subdir = ARCHITECTURES[arch_key][0]
                arch_name = subdir.split("/")[-1]
                for cfg in config_files:
                    local_path = os.path.join(msseg_dir, arch_name, cfg)
                    if not os.path.isfile(local_path):
                        self.progress.emit(f"Downloading {arch_name}/{cfg}...")
                        self._download_file(f"{arch_name}/{cfg}", local_path)

            # Download model checkpoints
            for i, (arch_key, fold) in enumerate(self._missing):
                subdir = ARCHITECTURES[arch_key][0]
                arch_name = subdir.split("/")[-1]
                display = ARCHITECTURES[arch_key][2]
                rel_path = f"{arch_name}/fold_{fold}/checkpoint_best.pth"
                local_path = os.path.join(msseg_dir, rel_path)

                self.progress.emit(f"[{i+1}/{total}] Downloading {display} fold {fold}...")
                self.download_progress.emit(int((i / total) * 100))

                self._download_file(rel_path, local_path)

            self.download_progress.emit(100)
            self.finished.emit(True, f"Downloaded {total} model(s) successfully.")
...
def _create_predictor_3d(model_dir, device_str, folds=(0,), use_mirroring=True):
    if nnUNetPredictor is None:
        raise RuntimeError("nnUNet is not available. Check the log for import errors.")
    if device_str == "cuda":
        try:
    ...
    from msseg.inference import _safe_initialize_predictor
...
class MSLesionApp(QMainWindow):
    def __init__(self):
        ...
        # Restore custom model directory from settings
        global _USER_MODEL_DIR, _MODEL_SEARCH_DIRS
        saved_model_dir = self._qsettings.value("model_dir", "")
        if saved_model_dir and saved_model_dir != _USER_MODEL_DIR:
            _USER_MODEL_DIR = saved_model_dir
            _MODEL_SEARCH_DIRS = [_APP_DIR, _USER_MODEL_DIR]
    ...
    def _start_preloading(self):
        ...
        if _check_models_missing():
            return  # Models not downloaded yet, skip preloading
    ...
    def _build_ui(self):
        ...
        # Quick-load dropdown: optional, pre-populated from MSSEG_QUICK_FOLDER if set
        _QUICK_FOLDER = os.environ.get("MSSEG_QUICK_FOLDER", "")
        self._quick_combo = QComboBox()
        self._quick_combo.setToolTip("Quick-load a patient from the test set")
        self._quick_combo.setMinimumWidth(120)
        _patient_ids = []
        if os.path.isdir(_QUICK_FOLDER):
            _seen = set()
            for _f in sorted(os.listdir(_QUICK_FOLDER)):
                if _f.endswith((".nii", ".nii.gz")):
                    _pid = "_".join(_f.split("_")[:2])  # e.g. MSL_0097
                    if _pid not in _seen:
                        _seen.add(_pid)
                        _patient_ids.append(_pid)
        self._quick_combo.addItems(_patient_ids)
        self._quick_combo.setCurrentIndex(0 if _patient_ids else -1)
        self._quick_combo_folder = _QUICK_FOLDER
        top.addWidget(self._quick_combo)

        btn_quick = QPushButton("▶")
        btn_quick.setFixedWidth(28)
        btn_quick.setToolTip("Load selected patient from test set")
        btn_quick.clicked.connect(self._quick_load_patient)
    ...
    # ── Data loading ─────────────────────────────────────────────────
    def _quick_load_patient(self):
        """Load the patient selected in the quick-load combo from the test-set folder."""
        pid = self._quick_combo.currentText()
        if not pid:
            return
        folder = self._quick_combo_folder
        flair_path = os.path.join(folder, f"{pid}_0000.nii.gz")
        t1_path    = os.path.join(folder, f"{pid}_0001.nii.gz")
        if not os.path.isfile(flair_path):
            self._status.showMessage(f"FLAIR not found: {flair_path}", 5000)
            return
        if not os.path.isfile(t1_path):
            self._status.showMessage(f"T1 not found: {t1_path}", 5000)
            return
        files = {"FLAIR": flair_path, "T1": t1_path, "T2": None, "MASK": None}
        # Auto-detect ground truth mask in same folder
        for _f in os.listdir(folder):
            up = _f.upper()
            if pid.upper() in up and any(k in up for k in ("MASK", "SEG", "LABEL", "WMH")):
                files["MASK"] = os.path.join(folder, _f)
                break
    ...
    def _open_patient(self):
        ...
        start_dir = getattr(self, '_quick_combo_folder', '')

        flair_path, _ = QFileDialog.getOpenFileName(
            self, "Select FLAIR Volume", start_dir, nii_filter)
        if not flair_path:
            return
    ...
    def _load_from_files(self, files, patient_folder):
        ...
        self._fold_probs = {}
        self._brain_alpha_mask = self._build_brain_alpha_mask()
    ...
    def _build_brain_alpha_mask(self):
        """Build a stable occupancy mask so 3D slice planes stop at the brain, not the volume box."""
        ref_vol = self._flair_vol if self._flair_vol is not None else self._display_vol
        if ref_vol is None:
            return None

        positive = ref_vol[ref_vol > 0]
        if positive.size == 0:
            return None

        # Use a global threshold so edge slices don't become full opaque rectangles.
        thr = max(float(np.percentile(positive, 20)) * 0.35, float(np.max(positive)) * 0.04)
        mask = ref_vol > thr

        if _HAS_SCIPY and binary_fill_holes is not None:
            try:
    ...
    def _update_gl_planes(self, view_idx):
        ...
        vol = self._display_vol
        mask_vol = self._brain_alpha_mask
        
        # Grab raw unflipped arrays so OpenGL maps match walking coords identically
        if view_idx == 0:
            sl = vol[ix, :, :]
            mask_sl = mask_vol[ix, :, :] if mask_vol is not None else None
        elif view_idx == 1:
            sl = vol[:, :, ix]
            mask_sl = mask_vol[:, :, ix] if mask_vol is not None else None
        else:
        ...
        # Use a stable 3D brain mask so slice planes don't render as gray end-caps.
        alpha_map = np.full(sl.shape, 90, dtype=np.float32)
        if mask_sl is not None:
            alpha_map[~mask_sl] = 0
        else:
            alpha_map[sl < (sl_max * 0.05)] = 0

        # Fade to transparent near volume edges so extreme slices don't show as gray rectangles.
        D, H, W = vol.shape
        max_idx = [D, W, H][view_idx]
        edge_margin = max(1, max_idx * 0.12)
        if ix < edge_margin:
            fade = ix / edge_margin
        elif ix > max_idx - 1 - edge_margin:
            fade = (max_idx - 1 - ix) / edge_margin
        else:
            fade = 1.0
        fade = max(0.0, min(1.0, fade))
        alpha_map = (alpha_map * fade).astype(np.uint8)
    ...
    def _run_segmentation(self):
        ...
        # Verify model checkpoints exist — offer download if missing
        missing = _check_models_missing()
        if missing:
            dlg = QDialog(self)
            dlg.setWindowTitle("Models not found")
            lay = QVBoxLayout(dlg)
            lay.addWidget(QLabel(
                f"{len(missing)} model checkpoint(s) not found.\n"
                f"Download from HuggingFace ({HF_REPO_ID})?"))
            # Path picker row — restore last chosen path from settings
            saved_path = self._qsettings.value("model_dir", _USER_MODEL_DIR)
            path_row = QHBoxLayout()
            path_edit = QLineEdit(saved_path)
            path_edit.setMinimumWidth(350)
            btn_browse = QPushButton("Browse...")
            def _browse():
                d = QFileDialog.getExistingDirectory(dlg, "Choose model directory", path_edit.text())
                if d:
                    path_edit.setText(d)
            ...
            lay.addWidget(buttons)
            if dlg.exec() == QDialog.DialogCode.Accepted:
                chosen = path_edit.text().strip()
                if chosen and chosen != _USER_MODEL_DIR:
                    _USER_MODEL_DIR = chosen
                    _MODEL_SEARCH_DIRS = [_APP_DIR, _USER_MODEL_DIR]
                self._qsettings.setValue("model_dir", _USER_MODEL_DIR)
                self._start_model_download(missing)
            return
    ...
    # ── Model download ────────────────────────────────────────────────
    def _start_model_download(self, missing):
        self._btn_seg.setEnabled(False)
        self._pbar.setValue(0)
        self._pbar.show()
        self._lbl_progress_pct.setText("0%")
        self._lbl_progress_pct.show()
        self._dl_thread = ModelDownloadThread(missing, parent=self)
        self._dl_thread.progress.connect(lambda m: self._status.showMessage(m))
        self._dl_thread.download_progress.connect(self._on_dl_progress)
    ...
    def _on_dl_finished(self, success, msg):
        ...
        self._btn_seg.setEnabled(True)
        if success:
            self._status.showMessage(msg + " You can now run segmentation.")
